...
try {
    ...
    $purchasedate = isset($data['purchasedate']) ? date('Y-m-d', strtotime($data['purchasedate'])) : '';
    $invDate = isset($data['invDate']) ? date('Y-m-d', strtotime($data['invDate'])) : '';
    ...
    if ($head_exists) {
        ...
        if ($details_result->num_rows > 0) {
            while ($detail_row = $details_result->fetch_assoc()) {
                ...
                $mfgdate = isset($detail_row['mfgdate']) ? $detail_row['mfgdate'] : '';
                $expdate = isset($detail_row['expdate']) ? $detail_row['expdate'] : '';
                $batchno = isset($detail_row['batchno']) ? $detail_row['batchno'] : '';
    ...
    foreach ($items as $item) {
        ...
        if ($register_result->num_rows > 0) {
            ...
            $stmt_register->bind_param("sisidddisi", $purchasedate, $productCode, $itemName, $slno,
    ...
    $stmt_party->bind_param("isddiisiisi", $temp_ledger_slno, $purchasedate, $grandTotal, $new_balance,
    ...
    if ($prAccId > 0) {
        ...
        $stmt_purchase->bind_param("issddiisiisi", $temp_purchase_slno, $purchasedate, $supplier,

...
try {
    ...
    $accId = isset($data['vendorID']) ? intval($data['vendorID']) : 0;
    ...
    $check_head = $conn->prepare("SELECT AccId FROM PurchaseHead WHERE Slno = ?");
    $check_head->bind_param("i", $slno);
    $check_head->execute();
    ...
    if ($head_exists) {
        ...
        $query_head = "INSERT INTO PurchaseHead 
                       (Slno, Purchasedate, InvNo, InvDate, Supplier, AccId, LrNo, TotalQty, 
                        ...
                        TotalMrp, lessDiscount, Waybill, CST, CSTAmount, ETax, ETaxAmount)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        
        $stmt_head = $conn->prepare($query_head);
        $stmt_head->bind_param("issssiissddddsdisdddd",
            $slno, $purchasedate, $invNo, $invDate, $supplier, $accId, $lrNo, $totalQty,
            $totalGross, $vatAmount, $netAmount, $rValue, $grandTotal, $freight,
            $totalMrp, $lessDiscount, $waybill, $cst, $cstAmount, $eTax, $eTaxAmount
        );
    ...
    foreach ($items as $item) {
        ...
        $freeQty = floatval($item['freeQty']) ?? 0;
        ...
        $spDiscount = floatval($item['spDiscount']) ?? 0;
        $discountAmount = floatval($item['discountAmount']) ?? 0;
        $cd = floatval($item['cd']) ?? 0;
        ...
        $stmt_details->bind_param("isidddddiddddddddsss",
        ...
        $check_stock = $conn->prepare("SELECT Qty FROM stock WHERE ProductCode = ?");
        $check_stock->bind_param("i", $productCode);
        $check_stock->execute();
        ...
        if ($stock_result->num_rows > 0) {
            ...
            $conn->query("UPDATE stock SET Qty = Qty + $totalQtyToAdd WHERE ProductCode = $productCode");
        } else {
            // Insert new stock
            $query_stock_insert = "INSERT INTO stock (itemname, MRP, PRate, Qty, ProductCode) 
                                   VALUES (?, ?, ?, ?, ?)";
            $stmt_stock_insert = $conn->prepare($query_stock_insert);
            $stmt_stock_insert->bind_param("sdddi", $itemName, $mrp, $rate, $totalQtyToAdd, $productCode);
        ...
        $check_stockdetails = $conn->prepare("SELECT Qty FROM stockdetails WHERE ProductCode = ? 
                                              AND mfgdate = ? AND expdate = ? AND batchno = ?");
        $check_stockdetails->bind_param("isss", $productCode, $mfgDate, $expDate, $batchNo);
        ...
        if ($stockdetails_result->num_rows > 0) {
            ...
            $conn->query("UPDATE stockdetails SET Qty = Qty + $totalQtyToAdd 
                         ...
                         AND expdate = '$expDate' AND batchno = '$batchNo'");
        } else {
            // Get item details from itemmaster
            $get_itemmaster = $conn->prepare("SELECT ProductType, itemtype, brand, barcode, size, Lose, unittype, SaleRate, HSN 
                                              FROM itemmaster WHERE ProductCode = ?");
            $get_itemmaster->bind_param("i", $productCode);
            ...
            if ($itemmaster_result->num_rows > 0) {
                ...
                $query_stockdetails = "INSERT INTO Stockdetails 
                                       (ProductType, itemtype, itemname, brand, barcode, size, MRP, PRate, Qty, 
                                        ProductCode, Vat, Lose, UniyType, SaleRate, mfgdate, batchno, expdate, hsn, purchaseinfo)
                                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                
                $stmt_stockdetails = $conn->prepare($query_stockdetails);
                $stmt_stockdetails->bind_param("ssssssdddiidssssss",
                    ...
                    $mfgDate, $batchNo, $expDate, $itemmaster['HSN'], $purchaseinfo
                );
        ...
        $check_register = $conn->prepare("SELECT id, closing FROM stockregister 
                                          WHERE VoucherType='Purchase' AND VoucherNo=? AND ProductCode=?");
        $check_register->bind_param("ii", $slno, $productCode);
        ...
        if ($register_result->num_rows > 0) {
            ...
            $last_register = $conn->prepare("SELECT Closing FROM stockregister 
                                             WHERE ProductCode=? AND id < ? ORDER BY id DESC LIMIT 1");
            $last_register->bind_param("ii", $productCode, $current_id);
            ...
            $conn->query("UPDATE stockregister SET openingqty=$last_closing, purchase=$totalQtyToAdd, 
                         closing=$new_closing WHERE id=$current_id");
            $conn->query("UPDATE stockregister SET openingqty=openingqty+$closing_diff, 
                         closing=closing+$closing_diff WHERE ProductCode=$productCode AND id > $current_id");
            
            $last_register->close();
        } else {
            // Insert new register entry
            $last_register = $conn->prepare("SELECT closing FROM stockregister 
                                             WHERE ProductCode=? ORDER BY Tdate DESC, id DESC LIMIT 1");
            $last_register->bind_param("i", $productCode);
            $last_register->execute();
            $last_result = $last_register->get_result();
            $last_closing = $last_result->num_rows > 0 ? $last_result->fetch_assoc()['closing'] : 0;
            ...
            $query_register = "INSERT INTO stockregister 
                              (Tdate, ProductCode, ProductName, VoucherType, VoucherNo, OpeningQty, 
                               Purchase, SaleRet, Sale, PurchaseRet, Closing, PartyID, PartyName)
                              VALUES (?, ?, ?, 'Purchase', ?, ?, ?, 0, 0, 0, ?, ?, ?)";
            
            $stmt_register = $conn->prepare($query_register);
            $stmt_register->bind_param("sisisidis", $purchasedate, $productCode, $itemName, $slno,
                                      $last_closing, $totalQtyToAdd, $new_closing, $accId, $supplier);
    ...
    $conn->query("DELETE FROM LedgerTran WHERE VoucherType='Purchase' AND VoucherSlno=$slno");
    
    // Get Purchase Account from LedgerMaster
    $purchase_acc = $conn->query("SELECT AccId, GroupId FROM LedgerMaster WHERE AccName LIKE 'Purchase%'");
    $prAccId = 0;
    ...
    $party_last = $conn->query("SELECT MAX(Slno) as max_slno, Balance FROM LedgerTran 
                               WHERE AccId=$vendorID ORDER BY Slno DESC LIMIT 1");
    $temp_ledger_slno = 1;
    ...
    $query_party_ledger = "INSERT INTO LedgerTran 
                          (Slno, TDate, Particulars, Dr, Cr, Balance, AccId, Remarks, VoucherType, 
                           VoucherSlno, GroupId, TranAccId, Username)
                          VALUES (?, ?, 'By Purchase', 0, ?, ?, ?, ?, 'Purchase', ?, ?, ?, ?)";
    
    $stmt_party = $conn->prepare($query_party_ledger);
    $groupId = 1; // Default group ID for vendor
    $stmt_party->bind_param("issdisiiis", $temp_ledger_slno, $purchasedate, $grandTotal, $new_balance,
                           $accId, $remarks, $slno, $groupId, $prAccId, $username);
    ...
    if ($prAccId > 0) {
        $purchase_last = $conn->query("SELECT MAX(Slno) as max_slno, Balance FROM LedgerTran 
                                      WHERE AccId=$prAccId ORDER BY Slno DESC LIMIT 1");
        ...
        $query_purchase_ledger = "INSERT INTO LedgerTran 
                                 (Slno, TDate, Particulars, Dr, Cr, Balance, AccId, Remarks, 
                                  VoucherType, VoucherSlno, GroupId, TranAccId, Username)
                                 VALUES (?, ?, ?, ?, 0, ?, ?, ?, 'Purchase', ?, ?, ?, ?)";
        
        $stmt_purchase = $conn->prepare($query_purchase_ledger);
        $stmt_purchase->bind_param("issddsisiiis", $temp_purchase_slno, $purchasedate, $supplier,
                                  $purchase_amount, $new_purchase_balance, $prAccId, $remarks, $slno,
                                  $purchaseGroupId, $accId, $username);
    ...
    if ($vatAmount > 0) {
        if ($stateCode == 21) {
            // SGST Entry
            $sgst_acc = $conn->query("SELECT AccId, GroupId FROM LedgerMaster WHERE AccName LIKE 'SGST%'");
            if ($sgst_acc && $sgst_acc->num_rows > 0) {
                ...
                $sgst_last = $conn->query("SELECT MAX(Slno) as max_slno, Balance FROM LedgerTran 
                                          WHERE AccId=".$sgst['AccId']." ORDER BY Slno DESC LIMIT 1");
                $sgst_slno = 1;
                $sgst_balance = 0;
                ...
                $stmt_sgst->bind_param("issddsisiiis", $sgst_slno, $purchasedate, $supplier, $sgst_amount,
                                      $new_sgst_balance, $sgst['AccId'], $remarks, $slno,
                                      $sgst['GroupId'], $accId, $username);
            ...
            // CGST Entry
            $cgst_acc = $conn->query("SELECT AccId, GroupId FROM LedgerMaster WHERE AccName LIKE 'CGST%'");
            if ($cgst_acc && $cgst_acc->num_rows > 0) {
                ...
                $cgst_last = $conn->query("SELECT MAX(Slno) as max_slno, Balance FROM LedgerTran 
                                          WHERE AccId=".$cgst['AccId']." ORDER BY Slno DESC LIMIT 1");
                $cgst_slno = 1;
                $cgst_balance = 0;
                ...
                $stmt_cgst->bind_param("issddsisiiis", $cgst_slno, $purchasedate, $supplier, $cgst_amount,
                                      $new_cgst_balance, $cgst['AccId'], $remarks, $slno,
                                      $cgst['GroupId'], $accId, $username);
            ...
            $igst_acc = $conn->query("SELECT AccId, GroupId FROM LedgerMaster WHERE AccName LIKE 'IGST%'");
            if ($igst_acc && $igst_acc->num_rows > 0) {
                ...
                $igst_last = $conn->query("SELECT MAX(Slno) as max_slno, Balance FROM LedgerTran 
                                          WHERE AccId=".$igst['AccId']." ORDER BY Slno DESC LIMIT 1");
                $igst_slno = 1;
                $igst_balance = 0;
                ...
                $stmt_igst->bind_param("issddsisiiis", $igst_slno, $purchasedate, $supplier, $vatAmount,
                                      $new_igst_balance, $igst['AccId'], $remarks, $slno,
                                      $igst['GroupId'], $accId, $username);

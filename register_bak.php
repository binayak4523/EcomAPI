...
if ($account_type === 'customer') {
    ...
    $address_stmt->close();
    //insert into ledgermaster table
    $insert_ledger_sql = "INSERT INTO ledgermaster (LedgerID, LedgerName, GroupID, DrCr, Type, Balance, Nature, Description, OpeningDate, ClosingDate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $ledger_stmt = $conn->prepare($insert_ledger_sql);    
    $ledger_stmt->bind_param("isssssssss", ...$account);
    if (!$ledger_stmt->execute()) {
        echo json_encode(["success" => false, "error" => "Failed to insert ledger account: " . $ledger_stmt->error]);
        break;
    }
//db.Execute ("insert into LedgerMaster (AccID,AccName,GroupID,Dr,Cr,TransactionType,OBalance,BalanceType,Groupname,Address1,Address2,statecode,tin) values(" & temp_accid & ",'" & Me.txtParty.Text & "'," & rec1("GroupID") & ",'+','-','" & temp_tran_type & "'," & Me.txtOpeningBalance.Text & ",'" & Me.cbodr_cr.Text & "','Sundry Debtor','" & Me.txtaddress1.Text & "','" & Me.txtAddress2.Text & "'," & Me.cbostate.ItemData(Me.cbostate.ListIndex) & ",'" & Me.txttin.Text & "')")

...
class PDF_Invoice extends FPDF {
    ...
    function Footer() {
        ...
        // Render signature on the left above the "Authorized Signatory" label; fallback to placeholder box
        $sigPathRaw = !empty($this->company['signature']) ? $this->company['signature'] : '';
        $sigPath = '';
        if ($sigPathRaw) {
            if (file_exists($sigPathRaw)) {
                $sigPath = $sigPathRaw;
            } elseif (file_exists(__DIR__ . '/' . ltrim($sigPathRaw, '/\\'))) {
                $sigPath = __DIR__ . '/' . ltrim($sigPathRaw, '/\\');
            }
        }

        $leftX = $this->lMargin; // left margin position
        $sigWidth = 60; // display width in mm
        $defaultSigBoxHeight = 25; // fallback box height in mm
        $y = $this->GetY();
        $usedHeight = $defaultSigBoxHeight;

        // If image exists, compute proportional height for given display width
        if (!empty($sigPath) && ($imgInfo = @getimagesize($sigPath))) {
            $imgW = $imgInfo[0];
            $imgH = $imgInfo[1];
            if ($imgW > 0) {
                $sigHeight = ($imgH / $imgW) * $sigWidth;
                // clamp reasonable height
                $sigHeight = min(max($sigHeight, 12), 60);
                $usedHeight = $sigHeight;
            } else {
                $usedHeight = $defaultSigBoxHeight;
            }
        } else {
        ...
        if (!empty($sigPath) && @getimagesize($sigPath)) {
            // draw signature image at left with computed height via width parameter
            $this->Image($sigPath, $leftX, $y, $sigWidth);
        } else {
            // draw placeholder rectangle and label at left
            $this->SetLineWidth(0.3);
            $this->Rect($leftX, $y, $sigWidth, $usedHeight);
            $this->SetFont('Arial', '', 8);
            $this->SetXY($leftX, $y + ($usedHeight / 2) - 3);
            $this->Cell($sigWidth, 6, 'Signature', 0, 0, 'C');
        }

        // move cursor just below the signature area (small gap) so Authorized Signatory appears directly below
        $this->SetY($y + $usedHeight + 2);
        $this->SetX($leftX);
        $this->SetFont('Arial','',9);
        $this->Cell($sigWidth,6,'Authorized Signatory',0,1,'C');
